...
> **Note:** This document captures historical context from the Claude Agent SDK reverse-engineering that informed the Copilot SDK migration. See `container/agent-runner/src/index.ts` for the current implementation using `@github/copilot-sdk`.

Findings from reverse-engineering the Claude Agent SDK v0.2.29–0.2.34 to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. These findings informed the architecture decisions for the Copilot SDK migration.
