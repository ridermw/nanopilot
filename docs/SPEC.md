...
│  │    • mcp__nanopilot__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│       └── update-nanopilot/SKILL.md   # /update-nanopilot - Upstream update merging
│
├── groups/
...
│   ├── nanopilot.log               # Host stdout
│   └── nanopilot.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nanopilot.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nanopilot-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nanopilot installation
...
NanoPilot uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Copilot SDK:
   ...
   └── mcpServers: nanopilot (scheduler)
   │
   ▼
...
NanoPilot has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__nanopilot__schedule_task]
        {
...
### NanoPilot MCP (built-in)

The `nanopilot` MCP server is created dynamically per agent call with the current group's context.
...
NanoPilot runs as a single macOS launchd service.

### Startup Sequence

When NanoPilot starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NanoPilot containers from previous runs
...
### Service: com.nanopilot

**launchd/com.nanopilot.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/nanopilot.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanopilot.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nanopilot.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanopilot` |
| "Agent process exited with code 1" | Container runtime failed to start | Check logs; NanoPilot auto-starts container runtime but may fail |
...
- `logs/nanopilot.log` - stdout
- `logs/nanopilot.error.log` - stderr
