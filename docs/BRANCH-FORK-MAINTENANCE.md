...
**`ridermw/nanopilot`** (upstream) — core engine with skill definitions (`.claude/skills/`). No channel code on `main`.

**Channel forks** (`nanopilot-whatsapp`, `nanopilot-telegram`, `nanopilot-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges nanopilot-whatsapp fork  → adds WhatsApp
...
```bash
# In your local nanopilot checkout
git checkout main && git pull

# For a fork:
git fetch nanopilot-whatsapp
git checkout -B whatsapp-merge nanopilot-whatsapp/main
git merge main
...
git push nanopilot-whatsapp HEAD:main
...
1. Fork `nanopilot` to `nanopilot-{channel}`
