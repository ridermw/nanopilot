# Running NanoPilot in Docker Sandboxes (Manual Setup)

This guide walks through setting up NanoPilot inside a [Docker Sandbox](https://docs.docker.com/ai/sandboxes/) from scratch — no install script, no pre-built fork. You'll clone the upstream repo, apply the necessary patches, and have agents running in full hypervisor-level isolation.
...
Enter the sandbox:
```bash
docker sandbox run shell-nanopilot-workspace
```
...
## Step 3: Clone and Install NanoPilot

NanoPilot must live inside the workspace directory — Docker-in-Docker can only bind-mount from the shared workspace path.
...
WORKSPACE=/Users/you/nanopilot-workspace

# Move into workspace so DinD mounts work
mv nanopilot "$WORKSPACE/nanopilot"
cd "$WORKSPACE/nanopilot"
...
NanoPilot needs several patches to work inside a Docker Sandbox. These handle proxy routing, CA certificates, and Docker-in-Docker mount restrictions.
...
In `src/container-runtime.ts`, the `cleanupOrphans()` function matches containers by the `nanopilot-` prefix. Inside a sandbox, the sandbox container itself may match (e.g., `nanopilot-docker-sandbox`). Filter out the current hostname:
...
```bash
cd ~ && git clone https://github.com/ridermw/nanopilot.git && mv nanopilot /path/to/workspace/nanopilot
```
