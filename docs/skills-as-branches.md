...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way NanoPilot is extended.

NanoPilot has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.nanopilot/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`ridermw/nanopilot`) maintains:

- `main` — core NanoPilot (no skill code)
...
- `/setup`, `/debug`, `/update-nanopilot`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`ridermw/nanopilot-skills`)
...
copilot plugin install nanopilot-skills@nanopilot-skills --scope project
```
...
NanoPilot's `.claude/settings.json` registers the official marketplace:
...
```
ridermw/nanopilot-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    nanopilot-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `nanopilot-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-nanopilot` — after merging main, optionally check for skill updates
...
1. Fork `ridermw/nanopilot` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/nanopilot.git
   cd nanopilot
   ```
3. Run the CLI:
   ```bash
...
Users who previously ran `git clone https://github.com/ridermw/nanopilot.git` and have local customizations:

1. Fork `ridermw/nanopilot` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/nanopilot.git
...
3. From this point, `origin` = their fork, `upstream` = ridermw/nanopilot
...
In both cases:
- Delete the `.nanopilot/` directory (no longer needed)
...
This is the same as the existing `/update-nanopilot` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-nanopilot` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to ridermw/nanopilot:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`ridermw/nanopilot-skills`)
...
1. Maintains a fork of NanoPilot (e.g., `alice/nanopilot`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/nanopilot-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to NanoPilot's `.claude/settings.json`:
...
```json
{
  "extraKnownMarketplaces": {
    ...
    "alice-nanopilot-skills": {
      "source": {
        "source": "github",
        "repo": "alice/nanopilot-skills"
      }
    }
  }
}
...
Once merged, all NanoPilot users automatically discover the community marketplace alongside the official one.
...
```bash
copilot plugin install alice-skills@alice-nanopilot-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/nanopilot.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/nanopilot-skills` to discover skills from any source.
...
A flavor is a curated fork of NanoPilot — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "NanoPilot for Sales," "NanoPilot Minimal," "NanoPilot for Developers").
...
AskUserQuestion: "Start with a flavor or default NanoPilot?"
- Default NanoPilot
- NanoPilot for Sales — Gmail + Slack + CRM (maintained by alice)
- NanoPilot Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/nanopilot.git
...
- `upstream` — `ridermw/nanopilot` (core updates)
...
```yaml
flavors:
  - name: NanoPilot for Sales
    repo: alice/nanopilot
    description: Gmail + Slack + CRM integration, daily pipeline summaries
    maintainer: alice

  - name: NanoPilot Minimal
    repo: bob/nanopilot
...
- **GitHub topics** — flavor forks can tag themselves with `nanopilot-flavor` for searchability
...
- `.nanopilot/` state directory

Operational skills (`setup`, `debug`, `update-nanopilot`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/ridermw/nanopilot.git
cd NanoPilot
copilot
```
...
```
1. Fork ridermw/nanopilot on GitHub
2. git clone https://github.com/<you>/nanopilot.git
3. cd nanopilot
4. copilot
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/ridermw/nanopilot.git`
- Check if `origin` points to the user's fork (not ridermw). If it points to ridermw, guide them through the fork migration.
- **Install marketplace plugin:** `copilot plugin install nanopilot-skills@nanopilot-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-nanopilot, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `copilot plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`ridermw/nanopilot-skills`) — single plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in NanoPilot. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.nanopilot/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.nanopilot/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to NanoPilot's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
