#!/bin/bash
# Watch NanoPilot IPC for Ollama activity and show macOS notifications
...
while true; do
  ...
  for status_file in data/ipc/*/ollama_status.json; do
    ...
    case "$STATUS" in
      generating)
        osascript -e "display notification \"$DETAIL\" with title \"NanoPilot → Ollama\" sound name \"Submarine\"" 2>/dev/null
        echo "$(date +%H:%M:%S) 🔄 $DETAIL"
        ;;
      done)
        osascript -e "display notification \"$DETAIL\" with title \"NanoPilot ← Ollama ✓\" sound name \"Glass\"" 2>/dev/null
        echo "$(date +%H:%M:%S) ✅ $DETAIL"
        ;;
      listing)
