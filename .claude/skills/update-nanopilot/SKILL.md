---
name: update-nanopilot
description: Efficiently bring upstream NanoPilot updates into a customized install, with preview, selective cherry-pick, and low token usage.
---

# About

Your NanoPilot fork drifts from upstream as you customize it. This skill pulls upstream changes into your install without losing your modifications.

Run `/update-nanopilot` in Claude Code.

## How it works

**Preflight**: checks for clean working tree (`git status --porcelain`). If `upstream` remote is missing, asks you for the URL (defaults to `https://github.com/ridermw/nanopilot.git`) and adds it. Detects the upstream branch name (`main` or `master`).
...
# Goal
Help a user with a customized NanoPilot install safely incorporate upstream changes without a fresh reinstall and without blowing tokens.
...
- Ask the user for the upstream repo URL (default: `https://github.com/ridermw/nanopilot.git`).
...
- Restart the service to apply changes:
  - If using launchd: `launchctl unload ~/Library/LaunchAgents/com.nanopilot.plist && launchctl load ~/Library/LaunchAgents/com.nanopilot.plist`
  - If running manually: restart `npm run dev`


## Diagnostics

1. Use the Read tool to read `.claude/skills/update-nanopilot/diagnostics.md`.
