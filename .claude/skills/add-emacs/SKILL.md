---
name: add-emacs
description: Add Emacs as a channel. Opens an interactive chat buffer and org-mode integration so you can talk to NanoPilot from within Emacs (Doom, Spacemacs, or vanilla). Uses a local HTTP bridge — no bot token or external service needed.
---

# Add Emacs Channel

This skill adds Emacs support to NanoPilot, then walks through interactive setup.
...
- **Code review** — select a region and send it with `nanopilot-org-send`; the response appears as a child heading inline in your org file
...
- **Schedule tasks** — ask Andy to set a reminder or create a scheduled NanoPilot task (e.g. "remind me tomorrow to review the PR")
...
If an `upstream` remote pointing to `https://github.com/ridermw/nanopilot.git` is missing,
add it:
...
- `emacs/nanopilot.el` — Emacs Lisp package (`nanopilot-chat`, `nanopilot-org-send`)
...
The `nanopilot.el` package requires only Emacs 27.1+ built-in libraries (`url`, `json`, `org`) — no package manager setup needed.
...
```elisp
;; NanoPilot — personal AI assistant channel
...
```elisp
(setq nanopilot-auth-token "<your-token>")
```

If `EMACS_CHANNEL_PORT` was changed from the default, also add:

```elisp
(setq nanopilot-port <your-port>)
```
...
Either a stale NanoPilot process is running, or 8766 is taken by another app.
...
Or change the port in `.env` (`EMACS_CHANNEL_PORT=8767`) and update `nanopilot-port` in Emacs config.

### No response from agent

Check:
1. NanoPilot is running: `launchctl list | grep nanopilot` (macOS) or `systemctl --user status nanopilot` (Linux)
2. Emacs group is registered: `sqlite3 store/messages.db "SELECT * FROM registered_groups WHERE jid = 'emacs:default'"`
3. Logs show activity: `tail -50 logs/nanopilot.log`

If the group is not registered, it will be created automatically on the next NanoPilot restart.
...
```elisp
;; M-x describe-variable RET nanopilot-auth-token RET
```

Must exactly match `EMACS_AUTH_TOKEN` in `.env`.

### nanopilot.el not loading

Check the path is correct:

```bash
ls ~/src/nanopilot/emacs/nanopilot.el
```

If NanoPilot is cloned elsewhere, update the `load`/`load-file` path in your Emacs config.
...
1. Delete `src/channels/emacs.ts`, `src/channels/emacs.test.ts`, and `emacs/nanopilot.el`
2. Remove `import './emacs.js'` from `src/channels/index.ts`
3. Remove the NanoPilot block from your Emacs config file
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.nanopilot` (macOS) or `npm run build && systemctl --user restart nanopilot` (Linux)