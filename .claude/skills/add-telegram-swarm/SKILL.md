...
1. *Share progress in the group* via `mcp__nanopilot__send_message` with a `sender` parameter matching their exact role/character name (e.g., `sender: "Marine Biologist"` or `sender: "Alexander Hamilton"`). This makes their messages appear from a dedicated bot in the Telegram group.
...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.nanopilot.plist`) in the `EnvironmentVariables` dict if using launchd.
...
>
> Check logs: `tail -f logs/nanopilot.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/nanopilot.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.nanopilot.plist && launchctl load ~/Library/LaunchAgents/com.nanopilot.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart nanopilot` (Linux)
