...
|-----|----------|---------|
| **Main app logs** | `logs/nanopilot.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/nanopilot.error.log` | Host-side errors |
...
```
**Fix:** Ensure a valid GitHub token is available on the host:
```bash
gh auth token  # Should print a token like gho_..., ghu_..., or github_pat_...
```
If missing or expired, re-authenticate with `gh auth login`. The token is injected into containers via stdin (`ContainerInput.githubToken`), not environment variables.
...
### 2. Token Not Reaching Container

**Runtime note:** `COPILOT_GITHUB_TOKEN` is passed via stdin JSON (`ContainerInput.githubToken`), NOT as an environment variable. The container reads the token from the stdin payload at startup.

To verify the token is available on the host:
```bash
gh auth token | head -c 10  # Should start with gho_, ghu_, or github_pat_
```
Token patterns (`gho_`, `ghu_`, `ghp_`, `github_pat_`) are automatically redacted from logs.
...
```bash
docker run --rm --entrypoint /bin/bash nanopilot-agent:latest -c 'ls -la /workspace/'
```

Expected structure:
```
/workspace/
├── group/                # Current group folder (cwd) — token arrives via stdin, not mounted
...
```bash
docker run --rm --entrypoint /bin/bash nanopilot-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash nanopilot-agent:latest
```
...
# Also clear the session ID from NanoPilot's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/nanopilot.log | tail -5
...
```bash
echo "=== Checking NanoPilot Container Setup ==="

echo -e "\n1. GitHub token available?"
gh auth token &>/dev/null && echo "OK ($(gh auth token | head -c 10)...)" || echo "MISSING - run: gh auth login"

echo -e "\n2. Token format valid?"
TOKEN=$(gh auth token 2>/dev/null); [[ "$TOKEN" =~ ^(gho_|ghu_|github_pat_) ]] && echo "OK" || echo "UNEXPECTED FORMAT - expected gho_*, ghu_*, or github_pat_*"
...
echo '{}' | docker run -i --entrypoint /bin/echo nanopilot-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/nanopilot.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
