---
name: claw
description: Install the claw CLI tool — run NanoPilot agent containers from the command line without opening a chat app.
---

# claw — NanoPilot CLI

`claw` is a Python CLI that sends prompts directly to a NanoPilot agent container from the terminal. It reads registered groups from the NanoPilot database, picks up secrets from `.env`, and pipes a JSON payload into a container run — no chat app required.
...
- Python 3.8 or later
- NanoPilot installed with a built and tagged container image (`nanopilot-agent:latest`)
- Either `container` (Apple Container, macOS 15+) or `docker` available in `PATH`

## Install

Run this skill from within the NanoPilot directory. The script auto-detects its location, so the symlink always points to the right place.
...
You should see registered groups. If NanoPilot isn't running or the database doesn't exist yet, the list will be empty — that's fine.
...
claw --image nanopilot-agent:dev "Hello"
...
The script auto-detects your NanoPilot directory and reads `.env` from it. Check that the file exists and contains `COPILOT_GITHUB_TOKEN`.

### Container times out

The default timeout is 300 seconds. For longer tasks, pass `--timeout 600` (or higher). If the container consistently hangs, check that your `nanopilot-agent:latest` image is up to date by running `./container/build.sh`.
...
### Override the NanoPilot directory

If `claw` can't find your database or `.env`, set the `NANOPILOT_DIR` environment variable:

```bash
export NANOPILOT_DIR=/path/to/your/nanopilot
```
