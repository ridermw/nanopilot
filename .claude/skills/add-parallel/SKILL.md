...
<!-- TODO: This skill needs validation with the Copilot SDK. Parallel agent spawning may work differently. -->

Adds Parallel AI MCP integration to NanoPilot for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NanoPilot scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NanoPilot already set up and running
...
Find the line:
```typescript
const allowedVars = ['COPILOT_GITHUB_TOKEN'];
```

Replace with:
```typescript
const allowedVars = ['COPILOT_GITHUB_TOKEN', 'PARALLEL_API_KEY'];
```
...
Add Parallel AI MCP servers after the nanopilot server:
```typescript
const mcpServers: Record<string, any> = {
  nanopilot: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__nanopilot__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__nanopilot__send_message
   4. Use mcp__nanopilot__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nanopilot-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep nanopilot  # macOS
# Linux: systemctl --user status nanopilot
```
...
- Check task runs: `tail -f logs/nanopilot.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.nanopilot` (macOS) or `systemctl --user restart nanopilot` (Linux)
