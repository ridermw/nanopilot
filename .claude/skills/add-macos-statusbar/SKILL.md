...
description: Add a macOS menu bar status indicator for NanoPilot. Shows a bolt icon with a green/red dot indicating whether NanoPilot is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows NanoPilot's running status and lets the user
...
```bash
launchctl list | grep com.nanopilot.statusbar
```
...
Create `~/Library/LaunchAgents/com.nanopilot.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.nanopilot.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.nanopilot.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see NanoPilot's status and control the service.
>
> - **Green dot** — NanoPilot is running
> - **Red dot** — NanoPilot is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.nanopilot.statusbar.plist
rm ~/Library/LaunchAgents/com.nanopilot.statusbar.plist
rm dist/statusbar
```
