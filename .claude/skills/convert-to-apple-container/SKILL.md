...
This skill switches NanoPilot's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
```bash
grep -q 'nanopilot proxy' /etc/pf.conf 2>/dev/null || echo '# nanopilot proxy — block LAN access to credential proxy
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo nanopilot-agent:latest "Container OK"
```
