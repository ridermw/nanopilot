...
> **Note:** `upstream` is the remote pointing to `ridermw/nanopilot`. If using a different remote name, substitute accordingly.
...
1. Start NanoPilot in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/nanopilot-test
cd /tmp/nanopilot-test
...
- **No config file.** NanoPilot's philosophy is customization through code changes, not configuration sprawl.
