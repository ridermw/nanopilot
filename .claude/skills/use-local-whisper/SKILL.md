...
The NanoPilot launchd service runs with a restricted PATH. `whisper-cli` and `ffmpeg` are in `/opt/homebrew/bin/` (Apple Silicon) or `/usr/local/bin/` (Intel), which may not be in the plist's PATH.

Check the current PATH:
```bash
grep -A1 'PATH' ~/Library/LaunchAgents/com.nanopilot.plist
```
...
```bash
tail -f logs/nanopilot.log | grep -i -E "voice|transcri|whisper"
```
