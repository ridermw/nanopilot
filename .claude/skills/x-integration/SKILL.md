---
name: x-integration
description: X (Twitter) integration for NanoPilot. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NanoPilot v1.0.0. Directory structure may change in future versions.
...
1. **NanoPilot is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep nanopilot (macOS) or systemctl --user status nanopilot (Linux)
```
...
| `NANOPILOT_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nanopilot.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NanoPilot, make the following modifications:
...
All paths below are relative to project root (`NANOPILOT_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep nanopilot  # macOS — should show PID and exit code 0 or -
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nanopilot.log | tail -20

# Script errors
grep -i "error\|failed" logs/nanopilot.log | tail -20
```
...
docker run nanopilot-agent ls -la /app/src/skills/
```
