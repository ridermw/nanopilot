...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo ridermw/nanopilot --search "<your feature>"
   gh issue list --repo ridermw/nanopilot --search "<your feature>"
   ```
...
NanoPilot uses skills — markdown files with optional supporting files that teach the assistant how to do something. There are four types of skills in NanoPilot, each serving a different purpose.
...
Add capabilities to NanoPilot by merging a git branch. The SKILL.md contains setup instructions; the actual code lives on a `skill/*` branch.
...
1. Fork `ridermw/nanopilot` and branch from `main`
...
**Examples:** `/setup`, `/debug`, `/customize`, `/update-nanopilot`, `/update-skills`
