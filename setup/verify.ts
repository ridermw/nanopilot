...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.nanopilot')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.nanopilot'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active nanopilot`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (fs.existsSync(envFile)) {
    ...
    if (/^COPILOT_GITHUB_TOKEN=/m.test(envContent)) {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'nanopilot', 'mount-allowlist.json'),
    )
  ) {
