...
<dict>
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanopilot');
  } catch {
...
/**
 * Kill any orphaned nanopilot node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned nanopilot processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nanopilot.service';
    ...
    unitPath = path.join(unitDir, 'nanopilot.service');
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanopilot processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable nanopilot`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nanopilot`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nanopilot`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanopilot.sh');
  const pidFile = path.join(projectRoot, 'nanopilot.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanopilot.sh — Start NanoPilot without systemd',
    ...
    '    echo "Stopping existing NanoPilot (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoPilot..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanopilot.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanopilot.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoPilot started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanopilot.log"`,
  ];
