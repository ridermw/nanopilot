...
describe('credentials detection', () => {
  it('detects COPILOT_GITHUB_TOKEN in env content', () => {
    const content =
      'SOME_KEY=value\nCOPILOT_GITHUB_TOKEN=gho_test123\nOTHER=foo';
    const hasCredentials =
      /^COPILOT_GITHUB_TOKEN=/m.test(content);
    expect(hasCredentials).toBe(true);
  });

  it('detects COPILOT_GITHUB_TOKEN with github_pat_ prefix', () => {
    const content = 'COPILOT_GITHUB_TOKEN=github_pat_abc123';
    const hasCredentials =
      /^COPILOT_GITHUB_TOKEN=/m.test(content);
    expect(hasCredentials).toBe(true);
  });
