...
// ---------------------------------------------------------------------------
// nanopilot--md-to-org-regex (Emacs Lisp, tested via emacs --batch)
...
function mdToOrg(input: string): string {
  const elFile = path.resolve('emacs/nanopilot.el');
  ...
  return execFileSync(
    ...
    [
      ...
      elFile,
      '--eval',
      `(princ (nanopilot--md-to-org-regex "${escaped}"))`,
    ],
    { encoding: 'utf8' },
  );
}

describe.skipIf(!emacsAvailable())('nanopilot--md-to-org-regex', () => {
  ...
  it('converts links [text](url) → [[url][text]]', () => {
    expect(mdToOrg('[NanoPilot](https://example.com)')).toBe(
      '[[https://example.com][NanoPilot]]',
    );
  });
});
